...
`netsh`. This mirrors the C# port, where the equivalents live under `Core/Native`
and are generated by CsWin32; the call sites choose between the native and
shell-out paths with `#[cfg(windows)]`, matching the C# `#if WINDOWS`.
