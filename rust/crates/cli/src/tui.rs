...
use pinnacle_core::platform::{Platform, TaskSpec};
use pinnacle_core::ui;

use crate::Host;

/// The tasks this build offers, in the order a run executes them.
///
/// The menu used to keep its own copy of this list, which meant a task could be
/// added to the CLI and forgotten here - invisible to anyone who double-clicks
/// `RUN.bat`, which is most users. It now reads the same table the argument
/// parser and the help text read, so that cannot happen.
...
fn show_banner() {
    ...
    ui::markup_line(&format!(
        "[bold cyan]│[/]  [dim]{:<50}[/][bold cyan]│[/]",
        format!("{} security hardening for CyberPatriot images", Host::NAME)
    ));
    ui::markup_line("[bold cyan]╰────────────────────────────────────────────────────╯[/]");
    ui::write_line();
    ...
    if !is_administrator() {
        ui::markup_line(&format!(
            "[yellow]! Not running as {}. Most changes will be refused.[/]",
            Host::PRIVILEGED_ROLE
        ));
        // Two different remedies, so the hint has to come from the platform
        // too: telling a Linux user to right-click an executable is worse than
        // saying nothing.
        ui::markup_line(&format!("[dim]  {}[/]", Host::ELEVATION_HINT));
        ui::write_line();
    }
}
...
/// probe on Windows, the effective uid on Linux.
...
fn warn_about_missing_readme(readme_args: &[String], chosen: &[String]) {
    ...
    let affected: Vec<&TaskSpec> = tasks()
        .iter()
        .filter(|spec| spec.needs_readme)
        .filter(|spec| all || chosen.iter().any(|c| c == spec.flag))
        .collect();
    ...
    for spec in affected {
        ui::markup_line(&format!("[yellow]    {}[/]", spec.label));
    }
}
...
mod tests {
    ...
    /// Every menu entry names a flag the CLI accepts, because both read the
    /// same table. This pins the shape the menu relies on: numbered selection
    /// needs a stable, non-empty, duplicate-free list.
    #[test]
    fn the_menu_offers_a_well_formed_task_for_every_number() {
        ...
        for spec in tasks() {
            ...
            assert!(!spec.label.is_empty(), "{} has no label", spec.flag);
            assert!(!seen.contains(&spec.flag), "{} listed twice", spec.flag);
            seen.push(spec.flag);
        }
        assert!(!seen.is_empty(), "the platform offers no tasks at all");
    }

    /// Pressing enter at the task prompt selects everything, so the flags it
    /// produces must all be real.
    #[test]
    fn selecting_everything_produces_one_flag_per_task() {
        let all: Vec<String> = tasks().iter().map(|t| t.flag.to_string()).collect();
        assert_eq!(all.len(), tasks().len());
        assert!(all.iter().all(|f| f.starts_with("--")));
    }

    /// The warning about a missing README is driven by the same rows as the
    /// menu, so a task cannot be warned about without also being offered.
    #[test]
    fn readme_dependent_tasks_are_offered_by_the_menu() {
        for spec in tasks().iter().filter(|s| s.needs_readme) {
            assert!(
                tasks().iter().any(|t| t.flag == spec.flag),
                "{} is warned about but never offered",
                spec.flag
            );
        }
    }
