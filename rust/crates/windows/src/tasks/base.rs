//! Shared helpers for the Windows tasks: reading local group membership and
//! parsing the console output of `net` and `ConvertTo-Csv`.
//!
//! The [`Task`](pinnacle_core::Task) trait itself lives in `pinnacle-core`, so
//! that the run pipeline can drive Windows and Linux tasks through one contract.
